...
if __name__ == "__main__":
    ...
    src_model.centre_0 = af.GaussianPrior(
        mean=0.0,
        sigma=0.25
    )
    src_model.centre_1 = af.GaussianPrior(
        mean=0.0,
        sigma=0.25
    )
    src_model.z_centre = af.GaussianPrior(
        mean=16.0,
        sigma=2.0
    )
    src_model.intensity = af.LogUniformPrior(
        lower_limit=10**-2.0,
        upper_limit=10**+2.0
    )
    src_model.maximum_velocity = af.UniformPrior(
        lower_limit=25.0,
        upper_limit=400.0
    )
    src_model.velocity_dispersion = af.UniformPrior(
        lower_limit=0.0,
        upper_limit=100.0
    )
